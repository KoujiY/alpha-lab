...
6. 頁面上方有「收藏」icon toggle（Star 圖示，存 localStorage）、「加入組合」按鈕（展開兩步 wizard）、「相關分析報告」區塊（列出 `GET /api/reports?symbol={symbol}` 的結果）
   - 加入組合 wizard（Phase 9）：
     - Step 1：選一個既有組合作為基底
     - Step 2：預覽合併後的權重表，每列可手動覆寫；其他持股會按比例 re-normalize 保持合計 100%。超出 Soft limit（持股 > 20 / 單檔 > 40% / 單檔 < 0.5%）會即時顯示警告但**不阻擋**儲存
   - 確認送出後仍走 probe 檢查：若今日報價不齊會彈「部分持股報價不齊」dialog，依原因分類顯示引導（無資料 / 報價過時 / 今日未收盤），流程與儲存推薦組合一致
7. 股價走勢（Phase 9）改用 lightweight-charts 繪製 OHLC 蠟燭圖 + 成交量（台股慣例：上漲紅、下跌綠）；若資料點 < 10 筆會退回折線顯示
...
- **管理動作**（Phase 6，Phase 9 icon 化）：
  - Star icon 收藏切換（列表卡片與詳情頁皆可；hover 顯示 tooltip）
  - Trash icon 刪除 → 彈出 shadcn AlertDialog 二次確認；詳情頁刪除後自動返回列表
- 檔案實體路徑：
...
1. **教學密度**：三個 radio（完整 / 精簡 / 關閉），與 header 右上的 📖 toggle 同步。Phase 9 起由 shadcn RadioGroup 呈現，鍵盤操作 ↑↓ 可切換
2. **收藏股票**：顯示目前收藏清單，每筆右側有 X 圖示可移除；空清單會提示去 `/stocks` 加星
3. **離線報告快取**：顯示目前已快取的報告篇數；「清空快取」按鈕會彈 shadcn AlertDialog 二次確認（取消 / 清空）；保留在伺服器的檔案不會被動到，只清瀏覽器 IndexedDB
...
| 9 | UI 升級（shadcn/ui 元件化、lightweight-charts K 線、icon button、加入組合 wizard、soft limit warnings）✅ |
