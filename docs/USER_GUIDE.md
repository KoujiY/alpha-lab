...
2. 點擊持股列表下方「儲存此組合」按鈕
3. 後端以當日為 `base_date`，從 `prices_daily` 取各持股收盤價作為基準，寫入 `portfolios_saved` 表
4. 頁面底部「已儲存組合」清單會即時新增一筆

前提：`prices_daily` 表需有目標 symbol 在 `base_date` 的收盤價；若無，API 會回 400 — 請先執行 `daily_collect`（見多因子評分章節）。
