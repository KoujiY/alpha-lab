...
- **testid 命名**：wizard 內部 `wizard-step-1` / `wizard-step-2` / `wizard-back` / `wizard-cancel` / `wizard-confirm` / `wizard-row-<symbol>` / `wizard-weight-input-<symbol>` / `wizard-sum` / `wizard-warning-<code>` 都是 E2E 的錨；改前要同步搬 E2E
