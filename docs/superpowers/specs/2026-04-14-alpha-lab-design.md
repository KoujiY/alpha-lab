...
| 7 | 未開始 | 數據源與自動化 + 組合追蹤強化 | Yahoo Finance 備援數據源、新聞彙整（每週掃描）、每日自動簡報（`data/reports/daily/`）、`data/processed/` 計算後指標；**組合追蹤強化**：`portfolios_saved.parent_id` + `parent_nav_at_fork` 血緣欄位（讓「加入組合」建立的新組合能把前段 NAV 接續顯示為連續曲線）、`SavedHolding` schema 層 symbol 唯一性斷言 + 浮點 `\|sum(weights)-1\| < 1e-6` 容忍、「加入組合」流程接 `probe_base_date` + `allow_fallback` dialog（新 symbol 在 base_date 停牌 / 未上市時不再撞 500，改為比照儲存組合的警示對話框） |
