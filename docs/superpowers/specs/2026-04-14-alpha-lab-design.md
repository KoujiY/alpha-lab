...
| 4 | ✅ 完成（2026-04-17） | 功能 E | 推薦理由靜態模板、L2 詳解側邊面板（5 個初始 topic）、報告儲存後端（`data/reports/`、`index.json`、summaries）、`/reports` 回顧列表與細節頁、「儲存此次推薦為報告」按鈕；產業代碼→中文名稱已於 Pre-Phase 4 `twse_stock_info` collector 隨 TWSE `t187ap03_L` 寫入 `stocks.industry`（不需額外 mapping 表） |
...
| 8 | ✅ 完成（2026-04-19） | 頁面擴充 | `/stocks` 全市場瀏覽列表頁（搜尋 / 產業篩選 / 收藏置頂 / 跳個股頁；`/api/stocks` limit 上限 500→3000）、`/settings` 偏好中心（聚合教學密度 / 收藏管理 / 報告快取清理，不抽獨立 context）、`/reports` grid/timeline view toggle（`groupReportsByMonth` + `alpha-lab:reports-view-mode` localStorage、sticky 月份 heading）、新增 `useFavorites` 反應式 hook（`storage` event 跨 tab 同步）。產業中文化的 Phase 4 遺留 TODO 已由 Pre-Phase 4 `twse_stock_info` collector 解決，此 Phase 無需額外 mapping。 |
| 9 | ✅ 完成（2026-04-18） | UI 升級 | shadcn/ui primitives（Button / Dialog / AlertDialog / Tooltip / RadioGroup / Select / IconButton）+ lucide-react 導入；`PriceChart` 改用 `lightweight-charts` v5 繪製蠟燭 + 成交量（台股紅漲綠跌；< 10 筆退折線）；列表 / 卡片 / 詳情的 ☆★ / 刪除 / view toggle 全部 icon 化（`data-testid` + `aria-label` + `aria-pressed` 保留不變）；`window.confirm` 改走 shadcn AlertDialog（新增 `*-confirm` / `*-cancel` / `*-proceed` testid）；`BaseDateConfirmDialog` / `RadioGroup` / `Select` 改走 Radix primitives；新增兩步 **AddToPortfolioWizard**（選基底 → 合併預覽表 + 手動覆寫單檔權重 + 其他持股等比 re-normalize + 即時 soft-limit 警告）；新增 **`lib/weightAdjust.ts`**（`rebalanceAfterEdit` + `isWeightSumValid` 純函式）與 **`lib/softLimits.ts`**（`checkSoftLimits` 回傳 `too_many_holdings` / `single_weight_too_high` / `weight_too_small` warnings）；`PortfoliosPage` 儲存推薦組合前也會做 soft-limit pre-check（有警告彈 dialog 讓使用者選「仍要儲存」或「取消」，不 hard block） |
